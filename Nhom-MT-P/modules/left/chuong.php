    ...
    <nav aria-label="...">
      	...
      	<?php
			...
			$idtheloai = $dong["idtheloai"];
	  		$thutu = $dong["thutu"];
	  		$chuongtiep = $thutu + 1;
            $sql2 = "SELECT * FROM chuong ch INNER JOIN truyen tr ON ch.idtruyen = tr.idtruyen WHERE  ch.thutu ='$chuongtiep'  AND tr.idtruyen = '$idtruyen'";      
			$tiep = mysql_fetch_array(mysql_query($sql2));

			$chuongtruoc = $thutu - 1;
            $sql3 = "SELECT * FROM chuong ch INNER JOIN truyen tr ON ch.idtruyen = tr.idtruyen WHERE  ch.thutu ='$chuongtruoc' AND tr.idtruyen = '$idtruyen'";
			$truoc = mysql_fetch_array(mysql_query($sql3));
	  	?>
        	<?php
            if($thutu == '1' || $truoc == false){
            ?>
